#!/usr/bin/env bash
# nRouter Wire Contract Pure-Curl Health Check Shell Runner
#
# Compares the live wire against spec/nrouter-sdk-spec.json, which every SDK in
# this repository is generated and tested against.
#
# Usage:
#   bash scripts/curl_health_checks/contract_curl.sh --self-test
#   bash scripts/curl_health_checks/contract_curl.sh --quick
#   bash scripts/curl_health_checks/contract_curl.sh
#   bash scripts/curl_health_checks/contract_curl.sh --step-summary
...
python3 "${SCRIPT_DIR}/contract_curl.py" "$@"
