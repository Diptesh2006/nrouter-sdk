...
def main() -> int:
    ...
    print("=== Starting nRouter Feature-by-Feature Pure-Curl Health Check ===")
    ...
    for check in result["checks"]:
        ...
        print(f"[{st}] #{check['index']} [{check['category']}] {check['name']} - HTTP {check['http_status']} ({check['latency_ms']}ms)")
        print(f"       Endpoint:    {check['method']} {check['endpoint']}")
        print(f"       Parameters:  {params_str}")
        print(f"       Response:    {check['response_snippet']}")
        if not check["passed"] and check.get("error"):
            print(f"       Error:       {check['error']}")

    print("------------------------------------------------------------")
    print(f"Total Feature Probes: {result['total_features']}")
        ...
        if step_summary_path:
            ...
            try:
                with open(step_summary_path, "a") as f:
                    f.write("\n" + md_content + "\n")
                print(f"Appended feature markdown summary to GITHUB_STEP_SUMMARY ({step_summary_path})")
            except Exception as exc:
