...
`POST /v1/mcp` speaks JSON-RPC 2.0 and is reached with the SAME customer virtual
key as inference — so the credential boundary is part of the contract, not an
afterthought.
...
class McpCurlHealthCheck:
    ...
    def _prepare(
        ...
        args: List[str] = []
        shown = [f'curl -sS -D - -X POST "$NROUTER_BASE_URL{MCP_PATH}"']
        if key is not None:
