#!/usr/bin/env bash
# nRouter MCP Pure-Curl Health Check Shell Runner
#
# Usage:
#   bash scripts/curl_health_checks/mcp_curl.sh --self-test
#   bash scripts/curl_health_checks/mcp_curl.sh --quick
#   bash scripts/curl_health_checks/mcp_curl.sh
#   bash scripts/curl_health_checks/mcp_curl.sh --step-summary
#
# Optional plane configuration:
#   NROUTER_MCP_SERVER           the server name to send as x-nr-mcp-server;
#                                without it the happy-path checks report
#                                NOT-CONFIGURED rather than PASS
#   NROUTER_CONTROL_PLANE_KEY    a management credential, to prove it is REFUSED
...
python3 "${SCRIPT_DIR}/mcp_curl.py" "$@"
