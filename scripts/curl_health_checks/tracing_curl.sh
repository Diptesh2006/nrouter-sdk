#!/usr/bin/env bash
# nRouter Request Identity & Tracing Pure-Curl Health Check Shell Runner
#
# Usage:
#   bash scripts/curl_health_checks/tracing_curl.sh --self-test
#   bash scripts/curl_health_checks/tracing_curl.sh --quick
#   bash scripts/curl_health_checks/tracing_curl.sh
#   bash scripts/curl_health_checks/tracing_curl.sh --step-summary
...
python3 "${SCRIPT_DIR}/tracing_curl.py" "$@"
